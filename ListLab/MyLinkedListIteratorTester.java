...
/**
 * A tester for the MyLinkedList generic class, focusing on iterators and various
...
public class MyLinkedListIteratorTester
{
     ...
     * 
     * @param args the standard args passed from terminal
     */
    public static void main(String[] args)
    {
        ...
        for(int i = 0; i < 500; i++)
        {
            ...
            if(div == 0 || div == 1)
            {
                ...
                for(int j = 0; j < 5; j++) 
                {
                    toAdd[j] = new Integer((i*(int)(Math.sin(i*i+j)*1000)*i+i*i)/
            ...
            { 
                ...
                for(int j = origSize - 1; j > (origSize / 2); j--)
                {
                    ...
                    {
                        throw new RuntimeException("Your list was the wrong size "
                            + "after removing node at index " + j);
                    }
                }
            } 
            else if(div == 5)
            ...
            else if(div == (NUM_TEST_TYPES - 1))
            {
                ...
                for(int j = orig.size() - 1; j > 0; j--)
                {
                    ...
                    if(!list.toString().equals(orig.toString()))
                    {
                        throw new RuntimeException("Iterator.remove() "
                            + "function failed at index " + j);
                    }
                    if(list.size() != orig.size())
                    {
                        throw new RuntimeException("Your list was the wrong size "
            ...
            if(list.size() != orig.size())
            {
                throw new RuntimeException("Your list is the wrong size "
                    + "(should have been " + orig.size() + ").");
            }
            ...
            {
                throw new RuntimeException("Check your iterator and linked "
                    + "list implementation, and also your add*() and remove*() methods.");
            }
        }
        System.out.println("\n\nCONGRATULATIONS! Your "
            + "LinkedList implementation's iterators and add and "
            + "remove methods work (or at least, most of them).");
    }
}
